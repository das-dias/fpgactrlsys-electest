...
async def test_fifo_cache_random(dut: Any):
    ...
    output, out_reg, curr_state, curr_size, curr_wptr, curr_rptr = fifo_cache_model(
        inputs[0], 0, np.zeros(DEPTH, dtype=int), 0, 0, 0, 
        reset_events[0], write_events[0], read_events[0], depth=DEPTH
    )
    ...
    for i, input_val in enumerate(inputs[1:], 1):
        output, out_reg, curr_state, curr_size, curr_wptr, curr_rptr = fifo_cache_model(
            input_val, out_reg, curr_state, curr_size, curr_wptr, curr_rptr, 
            reset_events[i], write_events[i], read_events[i], depth=DEPTH)
        #pdb.set_trace()
    ...
    for i, test_input in enumerate(inputs):
        ...
        await Timer(100, unit='ps')
        dut.clk.value = 0
        if int(dut.d_out.value) != int(expected_outputs[i]):
            pdb.set_trace()
        assert int(dut.d_out.value) == int(expected_outputs[i]), \
